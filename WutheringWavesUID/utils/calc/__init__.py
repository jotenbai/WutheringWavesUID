...
class WuWaCalc:
    def __init__(
        ...
        enemy_detail: EnemyDetailData | None = None,
    ):
        """
        ...
        self.role_card = {}
        # attr
        self.damageAttribute: DamageAttribute | None = None
        # 敌人
        if not enemy_detail:
    ...
    def enhance_summation_card_value(
        ...
        if weapon_result.sub_effect:
            ...
            card_sort_map[sub_effect_name] = sum_percentages(weapon_result.sub_effect["value"], card_sort_map[sub_effect_name])
        ...
        card_sort_map["普攻伤害加成"] = sum_percentages(result.get("普攻伤害加成", "0%"), card_sort_map.get("普攻伤害加成", "0%"))
        card_sort_map["attack_damage"] = percent_to_float(card_sort_map["普攻伤害加成"])

        card_sort_map["重击伤害加成"] = sum_percentages(result.get("重击伤害加成", "0%"), card_sort_map.get("重击伤害加成", "0%"))
        ...
        card_sort_map["治疗效果加成"] = sum_percentages(result.get("治疗效果加成", "0%"), card_sort_map.get("治疗效果加成", "0%"))
