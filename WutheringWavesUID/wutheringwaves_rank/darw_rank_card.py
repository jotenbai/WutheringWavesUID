...
async def draw_rank_img(bot: Bot, ev: Event, char: str, rank_type: str) -> str | bytes:
    ...
    for index, temp in enumerate(zip(rankInfoList, results)):
        ...
        region_draw.rounded_rectangle([0, 0, 50, 20], radius=6, fill=rank.server_color + (int(0.9 * 255),))
    ...
    # 备注
    rank_row_title = "入榜条件"
    rank_row = "1.本服务器内绑定并录入该角色面板的用户（含已完成群归属后的 Discord 私聊）"
